...
const getUser = (email: string, ..._: any) => ({ email, username: "John" })
...
export default defineFormActions({
  signIn: async (event) => {
    ...
    if (!validValue(user.username)) return actionResponse(event, { email, incorrect: true }, { error: { message: "No user found" } })
